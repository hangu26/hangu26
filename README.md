<img src="assets/welcome.gif" alt="welcome" width="750"/>

# AI 시대에 발맞춰 성장하는 개발자, 이현구

Native Android Developer (Kotlin) · Web Developer (React)

---

## 🛠 Tech Stack

###  Android
<img src="https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=Kotlin&logoColor=white"> <img src="https://img.shields.io/badge/Android%20Studio-3DDC84?style=for-the-badge&logo=android-studio&logoColor=white">

###  Web
<img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=React&logoColor=black"> <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=JavaScript&logoColor=black"> <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white"> <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white">

###  Databases & Backend
<img src="https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black">

###  Tools & Design
<img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"> <img src="https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white"> <img src="https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white"> <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white">

---

## 💼 Work Experience

**EEG / PSG 데이터 시각화 Web App**  
Frontend Developer (React) | 2025.11 – 2026.02 | 대전대학교  
- EEG / PSG 데이터 전처리 및 분석  
- 사용자 친화적 그래프 UI 구현  
- 데이터 탐색 흐름 기반 UX 설계  
- DB 구조 설계 및 데이터 관리

**LungExercise (앱 외주)**  
Android Developer (Kotlin) | 2025.01 – 2025.11 | 개인 외주  
- 전자 마스크 + Galaxy Watch 연동 앱 개발  
- Bluetooth Classic(SPP) 기반 실시간 통신  
- Wear OS 연동 걸음 수 · 심박수 실시간 수집  
- UI / UX 전체 기획 및 디자인 (Figma)  
- 다중 디바이스 환경 UX 설계 및 예외 상황 처리

---

## 😷 Featured Project

**LungExercise** – Android + Wearable App  
- Bluetooth SPP 기반 전자 마스크 통신  
- Galaxy Watch 연동 실시간 데이터 수집  
- UI/UX 전체 기획 (Figma)  
- IoT 디바이스 데이터 통합 및 상태 관리  

**Tech:** `Kotlin` · `Android` · `Bluetooth SPP` · `Wear OS` · `Firebase` · `IoT` · `Figma`  
🔗 [GitHub Repository](https://github.com/hangu26/LungExercise)

---

## 🔌 IoT / Device Integration

📡 **Bluetooth Classic (SPP)** – Serial Communication  
⌚ **Galaxy Watch** – Wear OS Companion App  
🤖 **IoT Device** – Real-time Data Sync  

- 전자 마스크 ↔ Android 앱 시리얼 통신  
- Galaxy Watch ↔ 앱 실시간 활동 데이터 수집  
- 모바일 ↔ 디바이스 ↔ 웨어러블 통합 상태 관리  
- 연결 안정성 및 상태 피드백 UX 설계

---

## 📊 GitHub Activity

<img src="https://streak-stats.demolab.com?user=hangu26&theme=tokyonight&hide_border=true&card_width=400" />

---

## ✨ About Me

- 안드로이드 네이티브 앱 개발 (Kotlin)  
- React 기반 웹 개발  
- UI / UX 기획부터 구현까지 가능한 개발자  
- IoT · 웨어러블 디바이스 연동 경험  
- 멀티 디바이스 데이터 통합 및 상태 관리  
- Clean Code & 지속적인 개선 지향

🚀 *Build apps that users enjoy using.*
